...
- **`needs_ancillary` sub-classes.** Sub-class splits below 0.40 CV recall are kept for ecological fidelity but flagged for later resolution with ancillary covariates (topographic wetness index, phenology). In the current 34-class set S05/S09/S24 split and **S09.a/b are flagged**. Borderline candidates S04/S06/S08/S13 sit at ~0.25–0.30 gate recall and do not split under the deterministic fold scheme — they're the first to revisit once ancillary covariates land. Re-check `subclass_mappability.csv` after any reclustering.

...
- **2023 site numbers are stable by lookup.** `00_prep_2023.R` assigns integers 3001+ via committed `data/small_reference/site_numbers_2023.csv` — existing assignments are never renumbered, new site names append. Two taxonomy overrides live in `00_prep_2023.R` (GEUROS → Acomastylis rossii, VERCAL → Veratrum tenuipetalum), aligning the collaborator's crosswalk (`taxonomy_crosswalk_2023.csv`) with project canonicals. Open issue: cover-vs-polygon site-name mismatch (SH-1..4 vs JF-13..17) awaiting collaborator; mismatched sites are carried but only sites with both cover AND spectra enter clustering.
...
- **`needs_ancillary` sub-classes.** Sub-class splits below 0.40 CV recall are kept for ecological fidelity but flagged for later resolution with ancillary covariates (topographic wetness index, phenology). In the current 38-class set S01/S07/S08/S23/S25 split and **S08.a/b/c + S25.a/b are flagged**. The split set is unstable across reclusters (it changed with the 06_29 batch) — re-check `subclass_mappability.csv` after any reclustering.

# 23_class_change.R — class-group cover change across CRBU, 2018 -> 2025.
#
# Change-detection on the gated temporal product (22_temporal_gating.R):
# quantify a class group's cover in the gated 2018 map vs the 2025 map,
# overall and spatially; the RAW (ungated) 2018 map is summarized as the
# calibration contrast. Select the group via env CHESS_CHANGE_GROUP
# (default "willow"). Known artifact (user, 2026-08-29): the large gain
# patch in north-central CRBU is a suspected SHADOW ARTIFACT in the 2025
# spectrometer data — discount gains there.
#
# Inputs:
...
GROUPS <- list(
  willow   = list(classes = c("Salix other", "Salix wolfii"),
                  name = "Willow shrubland"),
...
cfg <- GROUPS[[grp]]
cat(sprintf("=== Change analysis: %s (%s) ===\n", cfg$name,
            paste(cfg$classes, collapse = " + ")))
...
willow_codes <- lookup$class_code[lookup$final_label %in% cfg$classes]
stopifnot(length(willow_codes) == length(cfg$classes))
...
cat(sprintf("%s area 2018 (raw):   %8.1f ha\n", cfg$name, a18r))
cat(sprintf("%s area 2018 (gated): %8.1f ha\n", cfg$name, a18g))
cat(sprintf("%s area 2025:         %8.1f ha\n", cfg$name, a25))
...
cat(sprintf("Gross loss: %.1f ha; gross gain: %.1f ha (net %+.1f ha)\n",
...
top_of(loss_r, c25,  "LOSS becomes (2025 class)")
top_of(gain_r, c18g, "GAIN was    (2018 class)")
...
terra::writeRaster(delta, sprintf("data/derived/%s_change_300m.tif", grp),
...
summary_tbl <- dplyr::bind_rows(
  tibble::tibble(stratum = "CRBU total", n_cells = nrow(strat),
                 area_2018_gated_ha = a18g, area_2025_ha = a25,
                 delta_ha = a25 - a18g, delta_pct = 100 * (a25 - a18g) / a18g,
                 gross_loss_ha = n_loss, gross_gain_ha = n_gain,
                 area_2018_raw_ha = a18r, group = grp, classes = paste(cfg$classes, collapse = "+")),
)
readr::write_csv(summary_tbl, sprintf("data/derived/%s_change_summary.csv", grp))
readr::write_csv(bands, sprintf("data/derived/%s_change_by_meltband.csv", grp))
...
p_map <- ggplot(dl, aes(x, y, fill = d)) +
  ...
  scale_fill_gradient2(low = "#8C510A", mid = "#F5F5F2", high = "#01665E",
                       ...
                       name = expression(Delta ~ "cover fraction")) +
  coord_equal(expand = FALSE) +
  labs(title = sprintf("%s change, CRBU 2018 → 2025", cfg$name),
...
p_hist <- ggplot(dl, aes(d)) +
  ...
  labs(x = expression(Delta ~ "cover fraction per 300 m cell"), y = "cells",
...
png(sprintf("docs/figures/%s_change_crbu.png", grp), width = 8, height = 10,
    units = "in", res = 300, bg = "white")
draw_both(); dev.off()
pdf(sprintf("docs/figures/%s_change_crbu.pdf", grp), width = 8, height = 10)
draw_both(); dev.off()
cat(sprintf("Wrote docs/figures/%s_change_crbu.{png,pdf}\n", grp))
